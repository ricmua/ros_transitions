...
if __name__ == '__main__':
    ...
    timestamp_records = []
    get_timestamp_ns = lambda: node.get_clock().now().nanoseconds
    class Model:
        ...
        def log_timestamp(self, event_data):
            record = dict(timestamp=get_timestamp_ns(), state=self.state)
            timestamp_records.append(record)
    state_transitions = [{**r, 'before': 'log_timestamp'} 
    ...
    # Compute the differences, as measured by the timer node, instead of the 
    # client. Compute the mean statistic.
    timestamps_inactive_ns = [r['timestamp'] 
                              for r in timestamp_records 
                              if (r['state'] == 'inactive')]
    elapsed_inactive_s = 1e-9 * numpy.diff(timestamps_inactive_ns)
    mean_inactive_ms = 1e3 * (elapsed_inactive_s - arguments.timeout_s).mean()
    ...
    record = \
      {'System ID': arguments.id if arguments.id else platform.node(),
       ...
       'Mean^* $\delta$ (ms)': f'{mean_inactive_ms:0.3f}',
      }
